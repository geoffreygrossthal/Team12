...
public class SceneController {
    ...
    public void switchToChefScene(ActionEvent event) throws IOException {
        ...
        if (Employee.verifyEmployeeID(passwordChef))
        {
    ...
    public void switchToProcessorScene(ActionEvent event) throws IOException {
        ...
        if (Employee.verifyEmployeeID(passwordProcessor))
        {
    ...
    public void refreshProcessor(ActionEvent event) throws IOException
    {
        ...
        chefAgent = new Chef_Agent("vien", 123);
    ...
    public void getID(ActionEvent event)
    {
        ...
        int id = Integer.parseInt(orderID);
        readyOrder.setText(processorAgent.getOrderString(id));
    }
    ...
    @FXML public void cookOrder(ActionEvent event) throws IOException
    {
        int id = Integer.parseInt(orderID);
        if(chefAgent != null)
        {
            if (chefAgent.getOrderStatus(id)==1)
            {
                chefAgent.updateStatus(id, 2);
                chefAgent.saveOrders();
            }
            else if (chefAgent.getOrderStatus(id)==2)
            {
                chefAgent.updateStatus(id, 3);
                chefAgent.saveOrders();
            }
        }
    }
    ...
    public void switchToOrderStatusScene(ActionEvent event) throws IOException {
        ...
        {
            String newID = WriteToFile.orderIDgen();
            String order = newID + " 0 " + pizzaType + " " + topping[0] + " " + topping[1] + " " + topping[2] + " " + topping[3] 
                    + " " + hr + mi + " " + passwordStudent;

            WriteToFile.addNewOrder(order);
            WriteToFile.writeOrderID(newID);
            barFloat = 33;
